...
CalibrateInc2 <- function(inc,
  ...
  NoP_calculated <- array(data = 0, dim = c(n_agegroups, n_states - 2), dimnames = list(
  ...
  for (gender in c("male", "female")) {
    ...
    for (starting_age in agegroups) {
      ...
      if (counter < n_agegroups) {
        ...
        for (i in 1:n_cycles) {
          tp.matrix["No_MASH", "Death", i] <- mat_tpDn[max(age, 1), gender]
          tp.matrix["No_MASH", "F0", i] <- min(1, inc * incRR[max(age, 1)]) * (1 - mat_tpDn[max(age, 1), gender])
          tp.matrix["No_MASH", "No_MASH", i] <- (1 - min(1, inc * incRR[max(age, 1)])) * (1 - mat_tpDn[max(age, 1), gender])
