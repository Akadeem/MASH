# Function body ----
##  scenario = "F0":
##              The model starts with a population with MASH.
##  scenario = "No_incidence":
##              The model starts with a population without MASH. No risk of developing MASH (A world without MASH).
##  scenario = "No_MASH":
##              The model starts with a population without MASH. There is a risk of developing MASH.
QALY_function_master <- function(calibrated_inc, scenario = "F0", gender = "male") {
  for (starting_age in c(5, 15, 25, 35, 45, 55, 65, 75, 90)) {
    age <- starting_age
    s_names <- c("No_MASH", "F0", "F1", "F2", "F3", "F4", "HCC", "DCC", "LT", "PLT", "Death")
    ...
    tp.matrix <- array(data = 0, dim = c(n_states, n_states, n_cycles), dimnames = list(
      s_names,
      s_names
    ))

    for (i in 1:n_cycles) {
      tp.matrix["No_MASH", "Death", i] <- mat_tpDn[age, gender]
      tp.matrix["No_MASH", "F0", i] <- min(1, inc * incRR[age]) * (1 - mat_tpDn[age, gender])
      tp.matrix["No_MASH", "No_MASH", i] <- (1 - min(1, inc * incRR[age])) * (1 - mat_tpDn[age, gender])
    ...
    age <- starting_age
    for (i in 1:n_cycles) {
      q_matrix[i, "No_MASH"] <- mat_utility[age, gender]
